...
class T5Dataset():
    def __init__(self, config_path, pretrained_name_or_path=None):
        ...
        if pretrained_name_or_path is None:
            ...
            self.tokenizer = T5Tokenizer.from_pretrained(pretrained_name_or_path)
    ...
    def length_count(self, name, lengths):
        ...
        max_length_5 = max(sorted_lengths[:k2])
        print(n, k, k2)
        print("{}: max_length={}, 90%_length={}, 50%_length={}, mean_length={}".format(
            name, max(lengths), max_length_9, max_length_5, sum(lengths) / n
        ))
...
def train_model(arg_path):
    ...
    if model_args.tokenizer_name_or_path:
        tokenizer = T5Tokenizer.from_pretrained(model_args.tokenizer_name_or_path, cache_dir=model_args.cache_dir)
    if model_args.model_name_or_path:
        if tokenizer is None:
            tokenizer = T5Tokenizer.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir)
        model = T5ForConditionalGeneration.from_pretrained(model_args.model_name_or_path, cache_dir=model_args.cache_dir)
    if model is None:
...
if __name__ == "__main__":
    ...
    # # test
    # arg_path = './args/t5_base_hotpot.json'
    # test_pt_path = './data/data_t5/hotpotqa/pt/test.pt'
    # model_dir = './model/t5-base/hotpotqa/'
    # tokenizer_dir = model_dir + 'tokenizer/'
    # result_save_path = './model/t5-base/hotpotqa/result/prediction.csv'
